...
const MsoNormalContent =doc.querySelectorAll( 'p.MsoNormal')
...
} else if (MsoNormalContent.length) {
  _content.value = parseReplyToContent(doc, 'p.MsoNormal')
}
...
<head>
  <style>
    ${css}
    .prose-f :where(p):not(:where([class~="not-prose"], [class~="not-prose"] *)) {
      margin-top: 0;
      margin-bottom: 0;
    }
    :root {
